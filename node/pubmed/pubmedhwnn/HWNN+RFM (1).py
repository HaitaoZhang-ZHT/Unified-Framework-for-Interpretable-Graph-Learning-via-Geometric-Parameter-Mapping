...
def parameter_parser():
    ...
    parser.add_argument("--filters", type=int, default=500, help="Filters in convolution.")
    # 注：划分比例改为硬编码（与RFM一致60/20/20），此处参数仅保留兼容性，实际未使用
    parser.add_argument("--test-size", type=float, default=0.2, help="Ratio of test samples (compatibility).")
    parser.add_argument("--valid-size", type=float, default=0.2, help="Ratio of validation samples (compatibility).")
    ...
    parser.add_argument("--weight-decay", type=float, default=0.0005, help="Adam weight decay.")
...
class HWNN(torch.nn.Module):
    ...
    def setup_layers(self):
        self.convolution_1 = HWNNLayer(
            ...
            K1=2, K2=2, approx=True, data=self.data, weight_init='pubmed_false_1_best_model_seed_42.npy'
        )
...
class HWNNTrainer(object):
    def __init__(self, args, features, target, data, seed):
        ...
        self.current_best_generalization_error = float('inf')
    ...
    def train_valid_test_split(self):
        ...
        # 打印分割信息（日志+控制台）
        logging.info(f"Data split (seed={DATA_SPLIT_SEED}, align with RFM): Train={len(train_nodes)}, Valid={len(valid_nodes)}, Test={len(test_nodes)}")
        print(f"Data split (align with RFM): Train={len(train_nodes)}, Valid={len(valid_nodes)}, Test={len(test_nodes)}")
    ...
    def evaluate_best_model(self, best_model_path):
        """加载最佳模型，评估test集最终性能（新增Precision、Recall）"""
    ...
    def fit(self):
        ...
        logging.info(f"Model selection criteria: Valid Accuracy > Valid Loss > Generalization Error (|train_acc - val_acc|)")
        ...
        # 训练结束后，记录最佳模型信息（新增Precision、Recall）
        logging.info(f"\n{'=' * 60}")
...
if __name__ == "__main__":
    ...
    data = Data()
    data.load(data_path='./data/pubmed/', data_name='pubmed')
