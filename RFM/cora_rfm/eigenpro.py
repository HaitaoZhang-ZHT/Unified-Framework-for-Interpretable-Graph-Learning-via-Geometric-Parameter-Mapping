...
class FKR_EigenPro(nn.Module):
    ...
    def eigenpro_iterate(self, samples, x_batch, y_batch, eigenpro_fn,
        ...
        self.weight.index_add_(0, batch_ids.long(), -eta * grad)

        # update fixed coordinate block (for EigenPro)
        kmat = self.kernel_fn(x_batch, samples)
        correction = eigenpro_fn(grad, kmat)
        self.weight.index_add_(0, sample_ids.long(), eta * correction)
        return

    def evaluate(self, x_eval, y_eval, bs, metrics=('accuracy',)):
        p_list = []
        ...
        y_pred_class = np.argmax(p_eval, axis=1)
        y_true_class = np.argmax(y_eval, axis=1)

        if 'accuracy' in metrics:
            eval_metrics['accuracy'] = np.mean(y_pred_class == y_true_class)
        # Add more classification metrics as needed

        return eval_metrics, p_eval
    ...
    def fit(self, x_train, y_train, x_val, y_val, epochs, mem_gb,
        ...
        if n_subsamples is None:
            if n_samples < 100000:
                n_subsamples = min(n_samples, 1600)
            else:
                n_subsamples = 12000
        ...
        sample_ids = self.tensor(sample_ids, dtype=torch.long)  # Ensure LongTensor type
        ...
        for epoch in epochs:
            ...
            for _ in range(epoch - initial_epoch):
                ...
                for batch_ids in np.array_split(epoch_ids, n_samples / bs):
                    ...
                    batch_ids = self.tensor(batch_ids, dtype=torch.long)  # Ensure LongTensor type
            ...
            if run_epoch_eval:
                ...
                tr_score,_ = self.evaluate(x_train_eval, y_train_eval, bs)
                tv_score,_ = self.evaluate(x_val, y_val, bs)
                _,p_eval = self.evaluate(x_val, y_val, bs)
                print("train accuracy: %.2f%%\tval accuracy: %.2f%% (%d epochs, %.2f seconds)\t" %
                      (tr_score['accuracy'] * 100,
                      tv_score['accuracy'] * 100,
                      epoch, train_sec))
