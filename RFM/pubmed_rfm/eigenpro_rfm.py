...
def hyperparam_train(X_train, y_train, X_test, y_test, c,
                     iters=5, ep_iter=10, L=10, normalize=False, M=None):
    ...
    # 新增逻辑：处理传入的 M 参数
    if M is None:
        M = np.eye(d, dtype='float32')  # 默认初始化
    else:
        M = M.copy()  # 使用传入的 M，避免外部引用被修改

    best_M = M.copy()  # 保存最佳 M 的副本
...
def train(X_train, y_train, X_test, y_test, c, M,
          iters=1, ep_iter=0, L=10, normalize=False):
