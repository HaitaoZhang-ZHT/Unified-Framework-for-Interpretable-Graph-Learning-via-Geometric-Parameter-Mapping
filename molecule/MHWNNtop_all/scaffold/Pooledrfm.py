...
from mrfmmore import RFM  # 导入修改后的RFM模型（返回原子重要性）
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.StreamHandler(stream=open(1, 'w', encoding='utf-8'))
    ]
)
...
def plot_atom_importance(first_mol_smiles, first_mol_importance_sorted, log_dir, max_atoms_to_show=20):
    ...
    for bar, value in zip(bars, importance_values):
        ...
        plt.text(width + 0.001 * max(importance_values), bar.get_y() + bar.get_height() / 2,
                 f'{value:.6f}', ha='left', va='center', fontsize=9)

    # 美化图形
    plt.xlabel('Atomic importance score', fontsize=12)
    plt.ylabel('Atomic index', fontsize=12)
    plt.title(
        f'The atomic importance of molecule in the test set{title_suffix}\nSMILES: {first_mol_smiles[:50]}{"..." if len(first_mol_smiles) > 50 else ""}',
        fontsize=13, pad=20)
...
def plot_atom_importance_heatmap(first_mol_smiles, first_mol_importance, log_dir):
    ...
    for i, value in enumerate(importance_values):
        ax2.text(i, 0, f'{value:.6f}', ha='center', va='center',
...
if __name__ == "__main__":
    ...
    # 超参数搜索参数
    parser.add_argument("--inner_iters", type=int, nargs="+", default=[13], help="内层迭代次数列表")
    parser.add_argument("--pooling_iters", type=int, nargs="+", default=[20], help="外层池化迭代次数列表")
    parser.add_argument("--bandwidth", type=float, nargs="+", default=[115], help="核带宽列表")
    parser.add_argument("--reg", type=float, nargs="+", default=[1e-1], help="正则化系数列表")
    parser.add_argument("--feature_vector_top_k", type=int, nargs="+", default=[9], help="保留特征向量数量列表")
