...
if __name__ == "__main__":
    ...
    # 超参数搜索参数
    parser.add_argument("--inner_iters", type=int, nargs="+", default=[12], help="内层迭代次数列表")
    parser.add_argument("--pooling_iters", type=int, nargs="+", default=[19], help="外层池化迭代次数列表")
    parser.add_argument("--bandwidth", type=float, nargs="+", default=[55], help="核带宽列表")
    parser.add_argument("--reg", type=float, nargs="+", default=[1e-1], help="正则化系数列表")
    parser.add_argument("--feature_vector_top_k", type=int, nargs="+", default=[3], help="保留特征向量数量列表")
